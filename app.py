...
# 1. ESTILO PROFESIONAL (Sin cursilerías)
...
# Usamos el nombre de modelo estándar sin prefijos extraños
...
if prompt := st.chat_input("Escriba su consulta..."):
    ...
    with st.chat_message("assistant"):
        # Instrucciones profesionales inyectadas directamente
        instrucciones = (
            f"Instrucción: Actúa como 'Chefcito', experto culinario (15 años de exp). Trato formal y respetuoso. "
            f"PROHIBIDO el lenguaje cursi. "
            f"Reglas: 1. Gramática perfecta. 2. Pregunta comensales si no lo dicen. "
        ...
        try:
            # Generación de contenido simple para evitar errores de conexión
                ...
                for chunk in response:
                    if chunk.text:
                        for word in chunk.text.split(" "):
                            yield word + " "
                            time.sleep(0.02) # Velocidad optimizada
